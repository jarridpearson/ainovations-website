...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const currentPortalAddonItem = subscription.items.data.find((item) =>
      Object.values(PORTAL_ADDON_PRICES).includes(item.price.id)
    );

    const currentAppAddonItem = subscription.items.data.find((item) =>
      Object.values(APP_ADDON_PRICES).includes(item.price.id)
    );
      ...
      if (requestedCredits > currentCredits) {
        if (subscription.pending_update) {
          const existingInvoiceState =
            await loadLatestInvoiceState(stripe, subscription);
