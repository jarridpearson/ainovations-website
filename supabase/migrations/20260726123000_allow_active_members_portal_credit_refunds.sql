...
begin
  ...
  if not exists (
    ...
    raise exception
      'Only an active organization member can receive this portal AI credit refund.';
  end if;
  ...
  if v_period_key is null or v_period_end is null then
    raise exception 'The organization billing period is not configured.';
  end if;
  ...
  if v_ledger_id is null then
    raise exception 'The organization portal-credit pool is not available.';
  end if;
  ...
  v_updated_used_credits :=
    greatest(coalesce(v_used_credits, 0) - p_credit_cost, 0);
