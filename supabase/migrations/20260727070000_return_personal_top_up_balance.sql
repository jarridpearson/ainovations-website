...
drop function if exists
  public.sync_personal_app_entitlements(uuid);

drop function if exists
  public.master_admin_set_personal_app_tier(text);
...
begin
  ...
  current_period_key := to_char(timezone('utc', now()), 'YYYY-MM');

  select
    case
      when lower(trim(coalesce(settings.effective_tier, 'free')))
        in ('free', 'starter', 'pro')
      then lower(trim(coalesce(settings.effective_tier, 'free')))
      else 'free'
    end,
    greatest(coalesce(settings.recurring_addon_allocation, 0), 0)
  into
    resolved_tier,
  ...
  select
    ...
    case
      when ledger.period_key = current_period_key
      then greatest(coalesce(ledger.one_time_top_up_balance, 0), 0)
      else 0
    end
  into
  ...
  existing_top_up_balance := coalesce(existing_top_up_balance, 0);
  ...
  do update set
    ...
    recurring_addon_allocation = excluded.recurring_addon_allocation,
    one_time_top_up_balance = excluded.one_time_top_up_balance,
...
begin
  ...
  recurring_addon_value := coalesce(recurring_addon_value, 0);
